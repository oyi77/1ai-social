...
class LarryPlaybookClient(BaseClient):
    ...
    def connect(self) -> None:
        ...
        logger.info("Larry Playbook client connected (skill-based, no external connection)")
    ...
    def generate_hooks(self, niche: str, count: int = 5) -> List[Hook]:
        ...
        for i in range(count):
            ...
            # Use example as base (in production, this would use AI to customize)
        ...
        logger.info(f"Generated {len(hooks)} hooks, best confidence: {hooks[0].confidence:.2f}")
        return hooks
    ...
    def get_confidence(self, hook_type: str) -> float:
        ...
        if hook_type in self._memory.get("learned_patterns", {}):
            adjustment = self._memory["learned_patterns"][hook_type].get("confidence_boost", 0.0)
    ...
    def select_best_formula(self, hooks: List[Hook]) -> Hook:
        ...
        logger.info(f"Selected best formula: {best_hook.type} (confidence: {best_hook.confidence:.2f})")
